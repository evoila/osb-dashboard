...
export class ChartDirective implements OnInit, OnChanges, OnDestroy {
  ...
  private fixAxes(axisName, optionsObj) {
    if (optionsObj.scales[axisName].length === 0) {
      return optionsObj;
    }
  ...
  private refresh(): void {
    ...
    if (
      (this.type === 'line' || this.type === 'bar' || this.type === 'radar') &&
      isComplete
    ) {
      // this.addScales(this.options);
      for (let i = 0; i < this.data.length; i++) {
        const colourDesc: Array<number> = [
        ...
        const colour =
          i < this.colours.length
            ? this.colours[i]
            : this.defaultsColours[i] || this.getColour(colourDesc);
      ...
      for (let i = 0; i < this.data.length; i++) {
        ...
        backgroundColors.push(
          i < this.colours.length
            ? this.colours[i]
            : this.backgroundColors[i] || this.rgba(colourDesc, 0.8)
        );
        hoverBackgroundColors.push(
          i < this.colours.length
            ? this.colours[i]
            : this.hoverBackgroundColors[i] || this.rgba(colourDesc, 0.8)
        );
      }
    ...
    this.options = { ...this.options, maintainAspectRatio: false };
    this.options = { ...this.options, responsive: true };
