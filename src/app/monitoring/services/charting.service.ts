...
export class ChartingService {
  ...
  private setCustomNamesForSeries(chart: Chart): boolean {
    if (!chart.esIndex || !chart.series || !chart.series.length) {
      return false;
    }
    // USING esIndex as a temporary workaround
    const customNames = chart.esIndex.split('<break>');

    if (customNames.length !== chart.series.length) {
      return false;
    }
  ...
  private plotBuckets(
    ...
    if (this.noSeriesCharts.indexOf(chart.type) > -1) {
      delete chart.series;
    }
    ...
    if (
      ...
      this.plotSingleValue(
        chart,
        bucket[0].value,
        `${queryType} ${query[0][queryType].field}`,
        1
      );
      return true;
    }
    ...
    if (!bucket[bucketIndex].buckets) {
      console.log(
        'Error filling chart - No buckets in given data - cancelling.'
      );
      return false;
    }
    if (!this.isNested) {
    ...
    bucket[bucketIndex].buckets.forEach((bucket: any, index: number) => {
      if (
        bucket[nestedIndex] != null &&
        bucket[nestedIndex].buckets != null &&
        ...
        const valueToUse =
          chart.fieldToCount === 'doc_count' ? doc_count : value;
  ...
  private sortData(chart: Chart): Chart {
    ...
    const useMonthNames =
      chart.aggregations[0].aggs.toString().indexOf(`"interval":"month"`) > -1;
    let sortedData: any = {};
    ...
    if (
      ...
      !series.length ||
      !labels ||
      !moment(labels[0], this.acceptedDateFormats, true).isValid()
    ) {
      return chart;
    }

    sortedData = this.sortByDate(
      this.destructureForSorting(labels, series, data),
      this.acceptedDateFormats
    );
    this.resetChart(chart);
  ...
  private destructureForSorting(
    ...
    labels.forEach((date, dateIndex) => {
      if (!forSorting[date]) {
        forSorting[date] = { date };
      }
      series.forEach((serieName, serieIndex) => {
        forSorting[date][serieName] =
          data[serieIndex] && data[serieIndex][dateIndex];
      });
    });
    return forSorting;
  }
  ...
  private bringBackStructureForChart(chart: Chart, sortedData: any): void {
    ...
    chart.series = Object.keys(sortedData[0]).filter(
      serieName => serieName !== 'date'
    );

    sortedData.forEach((byDateObj, dateIndex) => {
      chart.labels.push(byDateObj.date);
      chart.series.forEach((serieName, serieIndex) => {
        if (!chart.data[serieIndex]) {
          chart.data[serieIndex] = [];
        }
  ...
  fill(property: Array<any>, check: boolean, value: any) {
    if (check && property.indexOf(value) === -1) {
      property.push(value);
    }
    if (!check) {
      property.push(value);
